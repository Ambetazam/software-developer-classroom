...
SEPARATE CHANGES
       ...
       --interactive before you publish them. You can use git stash push --keep-index to run the test suite
...
MANAGING BRANCHES
   ...
   Graduation
       ...
       •   seen (patches seen by the maintainer) is an integration branch for things that are not quite ready for
           inclusion yet (see "Integration Branches" below).

       Each of the four branches is usually a direct descendant of the one above it.

       Conceptually, the feature enters at an unstable branch (usually next or seen), and "graduates" to master for
   ...
   Merging upwards
       ...
       Always commit your fixes to the oldest supported branch that requires them. Then (periodically) merge the
   ...
   Throw-away integration
       ...
       they interact. Perhaps the result of merging them does not even work? But on the other hand, we want to avoid
   ...
   Branch management for next and seen after a feature release
       ...
       The same rewind and rebuild process may be followed for seen. A public announcement is not necessary since
       seen is a throw-away branch, as described above.
...
DISTRIBUTED WORKFLOWS
   ...
   Merge workflow
       ...
       You will still have to tell people by other means, such as mail. (Git provides the git-request-pull(1) to send
       preformatted pull requests to upstream maintainers to simplify this task.)
       ...
       Occasionally, the maintainer may get merge conflicts when they try to pull changes from downstream. In this
       case, they can ask downstream to do the merge and resolve the conflicts themselves (perhaps they will know
...
SEE ALSO
       gittutorial(7), git-push(1), git-pull(1), git-merge(1), git-rebase(1), git-format-patch(1), git-send-email(1),
       git-am(1)
